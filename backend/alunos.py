...
from .models import (
    ...
    RankingAcademia,
    Aluno, AlunoCreate, AlunoOut, LoginAlunoRequest,
)

# Cria as tabelas no banco (inclui Aluno também)
Base.metadata.create_all(bind=engine)

app = FastAPI(title="GT Jiu - Aurevix MVP")

# CORS liberado para o frontend (GitHub Pages, etc.)
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
# token -> id_usuario (professor / usuário)
TOKENS: Dict[str, int] = {}
...
def gerar_codigo_aluno() -> str:
    """
    Gera um código de acesso para aluno no formato GTJ-123456.
    """
...
def criar_aluno(
    aluno: AlunoCreate,
    db: Session = Depends(get_db),
    user: Usuario = Depends(get_current_user)  # professor/usuário logado
):
    """
    Cria um aluno vinculado a uma academia e gera um código GTJ-XXXXXX.
    """
    codigo = gerar_codigo_aluno()
...
@app.post("/alunos/login", response_model=AlunoOut)
def login_aluno(payload: LoginAlunoRequest, db: Session = Depends(get_db)):
    """
    Login do aluno via código de acesso (sem precisar de token).
    """
    aluno = db.query(Aluno).filter(Aluno.codigo == payload.codigo).first()
